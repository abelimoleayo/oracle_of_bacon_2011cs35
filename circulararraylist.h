...
class CircularArrayList : public List<T> {
  ...
  public:
    CircularArrayList();          
    CircularArrayList(const CircularArrayList<T>& src);
    ~CircularArrayList();
    ...
    T getItem(int i);            // Returns the ith item in the list.

    void insertAtHead(T value);  // Prepends item to front of list.    
    void insertAtTail(T value);  // Appends item to back of list.
    void setItem(int i, T value);// Replaces the ith item in the list.
    T removeHead();              // Removes and returns front item.
    T removeTail();              // Removes and returns back item.
    T removeItem(int i);         // Returns and removes ith item in the list.
    T& operator[](int i);        // Returns a reference to ith item in list.

  private:
    void expandCapacity();       // Expands the array to store more items.
};

#include "circulararraylist.inl"  
