...
class List {
  public:
    ...
    virtual T getItem(int i) = 0;            // Returns item in ith position.
    ...
    virtual void setItem(int i, T value) = 0;// Replaces ith item in the list.
    virtual T removeHead() = 0;              // Removes and returns front item.
    virtual T removeTail() = 0;              // Removes and returns back item.
    virtual T removeItem(int i) = 0;         // Removes and returns ith item.
    virtual T& operator[](int i) = 0;        // Returns reference to ith item.
};
